...
describe 'DateRound', ->
  ...
  describe 'ceiling', ->
    ...
    ceiling = (date, unit) -> DateRound.ceiling(date, unit).toUTCString()
    
    it 'should ceiling the sample date', ->
      expect( ceiling(sampleDate, 'millisecond') ).to.equal( 'Sat, 05 Mar 2011 12:13:23 GMT' )
      expect( ceiling(sampleDate, 'second')      ).to.equal( 'Sat, 05 Mar 2011 12:13:23 GMT' )
      expect( ceiling(sampleDate, 'minute')      ).to.equal( 'Sat, 05 Mar 2011 12:14:00 GMT' )
      expect( ceiling(sampleDate, 'hour')        ).to.equal( 'Sat, 05 Mar 2011 13:00:00 GMT' )
      expect( ceiling(sampleDate, 'day')         ).to.equal( 'Sun, 06 Mar 2011 00:00:00 GMT' )
      expect( ceiling(sampleDate, 'week')        ).to.equal( 'Mon, 07 Mar 2011 00:00:00 GMT' )
      expect( ceiling(sampleDate, 'month')       ).to.equal( 'Fri, 01 Apr 2011 00:00:00 GMT' )
      expect( ceiling(sampleDate, 'year')        ).to.equal( 'Sun, 01 Jan 2012 00:00:00 GMT' )
  ...
  describe '*time', ->
    ...
    it 'should be equivalent to the non-time methods', ->
      ...
      expect( DateRound.ceilingTime( sampleTime, 'hour')    ).to.equal( 1299330000000 )
